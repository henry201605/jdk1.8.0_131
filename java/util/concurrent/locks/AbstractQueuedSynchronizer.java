...
public abstract class AbstractQueuedSynchronizer
    ...
    private void unparkSuccessor(Node node) {
        ...
        //头结点后的第一个线程
        Node s = node.next;
       //如果结点为空，或者状态waitStatus>0(CANCELLED=1),即线程为取消状态，剔除该线程
        if (s == null || s.waitStatus > 0) {
            s = null;
            /*1、从后向前查找状态为<=0的结点，要找到离头结点最近的一个，也就是说头结点的下一个状态为<= 0的结点
            *2、为什么要从后往前查找呢，如果直接从前往后查找是不是更快些呢？这里是有原因的：
            * 因为在添加队列，调用enq(final Node node)的时候，执行这句代码的时候 t.next = node;可能还没建立起来t和node的指向关系，也就是
            * 说，关系是残缺的，所以不能从前往后进行查找，这样会导致找不到下一个结点
             */
    ...
    private void doReleaseShared() {
         ...
         */
        /**
         * 共享锁的释放和独占锁的释放有一定的差别 前面唤醒锁的逻辑和独占锁是一样，先判断头结点是不是SIGNAL状态，
         * 如果是，则修改为0，并且唤醒头结点的下一个节点
         */
        for (;;) {
            Node h = head;
            if (h != null && h != tail) {
                ...
                *PROPAGATE：标识为PROPAGATE状态的节点，是共享锁模式下的节点状态，处于这个状态下的节点，会对线程的唤醒进行传播,比如CountDownLatch中的await()
                 */
                else if (ws == 0 &&
     ...
     * 比如现在队列中有 3 个线程处于阻塞状态，一旦 ThreadA被唤醒，并且设置为 head 之后，会继续唤醒后续的ThreadB
     */
     ...
     * 所以shouldParkAfterFailedAcquire会先判断当前节点的前驱是否状态符合要求，若符合则返回true，然后调用parkAndCheckInterrupt，
     * 将自己挂起。如果不符合，再看前驱节点是否>0(CANCELLED)，若是那么向前遍历直到找到第一个符合要求的前驱，若不是则将前驱节点的状态设置为SIGNAL。
...
BlockingQueue
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                //如果前驱是head,即该结点就是第一个存在线程的节点，那么便有资格去尝试获取锁
    ...
    public final void acquire(int arg) {
        /**
         * 1、tryAcquire():通过 tryAcquire()尝试获取独占锁，如果成功返回 true，失败返回 false；
         * 2、addWaiter():如果 try Acquire 失败，则会通过 addWaiter()方法将当前线程封装成 Node 添加到 AQS 队列尾部;
    ...
    public final boolean releaseShared(int arg) {
        //tryReleaseShared()用自旋的方法实现 state 减 1
    ...
    final boolean isOnSyncQueue(Node node) {
         ...
         */
        /**
         * 如果两个if都未返回时，可以断定node的prev一定不为null，next一定为null，这个时候可以认为node正处于放入Sync队列的执行CAS操作执行过程中。
         * 而这个CAS操作有可能失败，所以通过findNodeFromTail再尝试一次判断。
         * 链接：https://www.jianshu.com/p/52b07c88605e
         */
...
//尝试唤醒当前节点，并将当前节点移动到同步队列中
    final boolean transferForSignal(Node node) {
        ...
        /*
        *设置node的状态为0
        *1)、如果设置失败，说明已经被取消，没必要再进入Sync队列了，doSignal中的循环会找到一个node再次执行；
         *2)、如果设置成功，但之后又被取消了呢？无所谓，虽然会进入到Sync队列，但在获取锁的时候会调用shouldParkAfterFailedAcquire方法，
         * 该方法中会移除此节点。
         *
         **/
         ...
         */
        //则将node加入到Sync队列中,返回node的前继节点p
        Node p = enq(node);
        int ws = p.waitStatus;
        //在node的前继节点p节点是取消状态或者设置p节点的状态为SIGNAL失败的时候，
        // 才会执行LockSupport.unpark,从而唤醒当前node的线程。
        /*
    ...
    final boolean transferAfterCancelledWait(Node node) {
        ...
        if (compareAndSetWaitStatus(node, Node.CONDITION, 0)) {
            //则将node添加到Sync队列中，并返回true，表示中断在signal之前,也就是说唤醒之前就被中断了
            enq(node);
            return true;
        }
        /*
         ...
         * spin.
         */
        //检查当前线程的node是否已经在Sync队列中了，如果不在Sync队列中，则让步给其他线程执行，直到当前的node已经被signal方法添加到Sync队列中
        while (!isOnSyncQueue(node))//循环检测node是否已经成功添加到AQS队列中
//            如果没有，则通过yield,给其他线程让步
            Thread.yield();
        return false;
    }
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         */
        /**
         * 先将队列前面节点依次从队列中取出，然后调用transferForSignal方法去唤醒节点，这个方法有可能失败，
         * 因为等待线程可能已经到时或者被中断，因此while循环这个操作直到成功唤醒或队列为空。
         * @param first
         */
        private void doSignal(Node first) {
            ...
            do {
                ...
                first.nextWaiter = null;
                     ...
                     (first = firstWaiter) != null);//transferForSignal方法尝试唤醒当前节点，如果唤醒失败，则继续尝试唤醒
        }
         ...
         */
        /**
         * doSignal方法只是唤醒了一个node并加入到Sync队列中，而doSignalAll方法唤醒了所有的Condition节点，并加入到Sync队列中。
         */
         ...
         * storms.
         */
        //从头到尾遍历Condition队列，移除状态为非CONDITION的节点，因为在执行该方法时已经获取了独占锁，所以不需考虑多线程问题。
         ...
         */
        /**
         * 1、在await方法中可以知道添加到Condition队列中的node每次都是添加到队列的尾部，在signal方法中是从头开始唤醒的，
         * 所以Condition是公平的，signal是按顺序来进行唤醒的；
         * 2、在该方法中判断当前线程是否占有独占锁，然后通过firstWaiter依次唤醒Condition队列中的node，并把node添加到Sync队列中。
         */
        ...
        private int checkInterruptWhileWaiting(Node node) {
            //如果当前线程被中断，则调用transferAfterCancelledWait方法
            return Thread.interrupted() ?
...
//        根据interruptMode来确定是应该抛出InterruptedException还是继续中断
        private void reportInterruptAfterWait(int interruptMode)
            throws InterruptedException {
            //如果是在调signal()前就被中断则抛出异常
            if (interruptMode == THROW_IE)
                throw new InterruptedException();
            //如果是在调signal()方法后中断，就执行中断
        ...
        public final void await() throws InterruptedException {
            if (Thread.interrupted())
                throw new InterruptedException();
            //1、当前线程创建一个Node添加到Condition队列中
            Node node = addConditionWaiter();
            //2、释放当前线程的lock，从AQS的队列中移出
            int savedState = fullyRelease(node);
            int interruptMode = 0;
            //3、循环判断当前线程的Node是否在Sync队列中，如果不在，则park
            while (!isOnSyncQueue(node)) {
                LockSupport.park(this);
            ...
            //4、acquireQueued获取锁并返回线程是否中断
            // 如果线程被中断，并且中断的方式不是抛出异常，则设置中断后续的处理方式设置为REINTERRUPT
            if (acquireQueued(node, savedState) && interruptMode != THROW_IE)
                interruptMode = REINTERRUPT;
            //5、从头到尾遍历Condition队列，移除被cancel的节点
            if (node.nextWaiter != null) // clean up if cancelled
                unlinkCancelledWaiters();
            //6、如果线程已经被中断，则根据之前获取的interruptMode的值来判断是继续中断还是抛出异常
            if (interruptMode != 0)
